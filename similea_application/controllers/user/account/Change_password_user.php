...
class Change_password_user extends MY_Controller{
	...
	public function index(){		
		...
		if($this->form_validation->run())
		{
            ...
            if($this->account_model->change_password_users($username, $oldpassword, $newpassword1, $newpassword2))
			{
                $this->session->set_flashdata('error', 'Berhasil Mengubah Password');
				redirect('user/account/change_password_user');
			} else {
    ...
    public function show_data_user(){
		...
		echo "<br>";
        echo $password = $this->account_model->get_password_users($username);
    }		
}